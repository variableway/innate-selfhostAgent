...
| **Channels** | 查看已连接渠道的状态 |
| **Sessions** | 管理活跃会话和上下文 |
| **Skills** | 按状态过滤浏览技能（All / Ready / Needs Setup / Disabled），点击查看详情、安装依赖、设置 API Key |
| **Cron** | 查看和管理定时任务 |
...
macOS 专属的原生桌面应用（OpenClaw.app）：菜单栏常驻、原生通知、无需终端即可管理 Gateway。配置界面采用可折叠树形侧边栏导航，Agent 工作区文件支持展开式内联 Markdown 预览。
