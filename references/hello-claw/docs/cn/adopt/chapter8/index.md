...
为兼容更多客户端和 RAG 管道，Gateway 提供模型列表和嵌入端点：
...
通过 `/v1/chat/completions` 和 `/v1/responses` 端点的 `model` 字段可指定显式模型覆盖，Gateway 会透传给上游提供商。
...
如果 OpenClaw 运行在 Docker 或 Podman 容器中，可以用 `--container` 标志直接在容器内执行命令，无需手动 `docker exec`：
...
也可以通过环境变量设置：`OPENCLAW_CONTAINER=1`。
...
- `userDataDir not accessible` — 浏览器用户数据目录权限问题
