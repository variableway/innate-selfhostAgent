...
install_nodejs() {
    ...
    local lts_version=$(fnm ls 2>/dev/null | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' | head -1)
    if [ -n "$lts_version" ]; then
        fnm use "$lts_version"
        fnm default "$lts_version"
    fi
    
    local node_version=$(node -v 2>/dev/null || echo "安装中...")
    local npm_version=$(npm -v 2>/dev/null || echo "安装中...")
